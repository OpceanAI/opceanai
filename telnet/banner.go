...
// darkening from faint parchment shadow to full gall ink as the edge passes —
// the wordmark writes itself the way a scribe inks a title.
func drawBannerCells(c *Canvas, word string, col, row int, reveal float64) {
	...
	for y, line := range lines {
		for x, ch := range line {
			...
			a := clamp01(d / 12)
			fg := lerpRGB(pVellumLo, pInkDark, a)
